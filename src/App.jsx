...
// ---------------------------------------------------------------------------
// Water keyword matching
// Add phrases here to have them count toward daily water/hydration tracking.
// Matching is case-insensitive and substring-based (e.g. "herbal tea" matches
// "chamomile herbal tea"). The LLM is NOT used — only this list.
// ---------------------------------------------------------------------------
...
/**
 * Returns estimated fluid oz for a food description based solely on
 * WATER_KEYWORDS. Returns 0 if none of the keywords match.
 * Attempts to detect an explicit quantity (cups, glasses, oz, ml, litre).
 * Defaults to 8 oz (one standard cup/glass) when no quantity is found.
 */
export function detectWaterOz(description) {
  ...
  // Look for an explicit quantity with a unit
  ...
  // Look for a plain leading number without a unit ("2 herbal teas")
  const numMatch = lower.match(/^(\d+(?:\.\d+)?)\s+/)
  if (numMatch) return parseFloat(numMatch[1]) * 8

  return 8 // default: one standard serving = 8 oz
}
  ...
  const estimate = async () => {
    ...
    await Promise.all(parts.map(async (part, i) => {
      try {
        ...
        // Water is keyword-matched client-side, not estimated by the LLM
